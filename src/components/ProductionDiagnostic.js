...
const TestItem = styled.div`
    ...
    border-radius: 4px;
    background: ${props => 
        props.status === 'success' ? '#1a4d1a' : 
        props.status === 'error' ? '#4d1a1a' : 
        '#4d4d1a'
    };
...
export default function ProductionDiagnostic() {
    ...
    const runComprehensiveDiagnostic = async () => {
        ...
        addDiagnostic('Environment Check', 'pending', 'Checking environment variables...');
        
        const envVars = {
            'REACT_APP_PROJECT_ID': process.env.REACT_APP_PROJECT_ID,
            'REACT_APP_API_KEY': process.env.REACT_APP_API_KEY ? '***SET***' : 'MISSING',
        ...
        if (missingEnvVars.length === 0) {
            addDiagnostic('Environment Check', 'success', 'All environment variables present', envVars);
        } else {
            addDiagnostic('Environment Check', 'error', `Missing variables: ${missingEnvVars.join(', ')}`, envVars);
        }

        // Test 2: Firebase Config Validation
        addDiagnostic('Firebase Config', 'pending', 'Validating Firebase configuration...');
        ...
        try {
            ...
            addDiagnostic('Firebase Config', 'success', 'Firebase configuration valid', {
                projectId,
                authDomain,
        ...
        addDiagnostic('Database Instance', 'pending', 'Checking Firestore database instance...');
        ...
        try {
            ...
            addDiagnostic('Database Instance', 'success', 'Firestore instance initialized', dbInfo);
        } catch (error) {
            addDiagnostic('Database Instance', 'error', error.message);
        }

        // Test 4: Network Connectivity Test
        addDiagnostic('Network Test', 'pending', 'Testing basic connectivity...');
        ...
        try {
            ...
            if (response.status === 200 || response.status === 401 || response.status === 403) {
                addDiagnostic('Network Test', 'success', `Firestore API reachable (Status: ${response.status})`);
            } else {
                addDiagnostic('Network Test', 'error', `Unexpected response: ${response.status} ${response.statusText}`);
            }
        } catch (error) {
            addDiagnostic('Network Test', 'error', `Network error: ${error.message}`);
        }

        // Test 5: Firestore Collection Test
        addDiagnostic('Firestore Access', 'pending', 'Testing Firestore collection access...');
        
        try {
            const projectsRef = collection(db, 'projects');
            const snapshot = await getDocs(projectsRef);
            
            addDiagnostic('Firestore Access', 'success', `Successfully accessed projects collection (${snapshot.size} documents)`, {
                collectionPath: 'projects',
                documentCount: snapshot.size,
                empty: snapshot.empty
            });
        } catch (error) {
            addDiagnostic('Firestore Access', 'error', `Firestore access failed: ${error.message}`, {
        ...
        try {
            ...
            const testDocRef = doc(db, 'projects', 'test-document-that-does-not-exist');
            await getDoc(testDocRef);
            
            addDiagnostic('Security Rules', 'success', 'Security rules allow read access');
        } catch (error) {
            if (error.code === 'permission-denied') {
                addDiagnostic('Security Rules', 'error', 'Security rules deny access', {
                    errorCode: error.code,
                    suggestion: 'Update Firestore security rules to allow read access'
                });
            } else {
                addDiagnostic('Security Rules', 'success', 'Security rules allow access (document not found is expected)');
            }
        }
        ...
        <DiagnosticContainer>
            ...
            {diagnostics.map((diagnostic, index) => (
                <TestItem key={index} status={diagnostic.status}>
                    ...
                    {diagnostic.details && (
                        <pre style={{ margin: '0.5rem 0 0 0', fontSize: '0.8rem', opacity: 0.8 }}>
